...
public class ServeurBanque extends Serveur {
    ...
    public void supprimeInactifs() {
        //À définir :
        for (Connexion cnx:connectes) {
            if(((ConnexionBanque)cnx).estInactifDepuis(DELAI_INACTIVITE)==true){
            //if(System.currentTimeMillis()-((ConnexionBanque)cnx).getTempsDerniereOperation()>= DELAI_INACTIVITE){
                cnx.envoyer("END");
                cnx.close();
                super.enlever(cnx);
                System.out.println("La connexion est bien fermee et supprimee!");
                break;
            }
        }
    }
